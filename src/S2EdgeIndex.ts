...
export abstract class S2EdgeIndex {
   ...
   */
  private minimumS2LevelUsed: number;

  /**
   * Has the index been computed already?
   */
  private indexComputed:boolean ;
   ...
   */
  private static compare(cell1:Long, edge1:number, cell2:Long, edge2:number):number {
  if (cell1 < cell2) {
  return -1;
} else if (cell1 > cell2) {
  return 1;
...
public computeIndex() {
  ...
  for (let i = 0; i < this.getNumEdges(); ++i) {
    const from = this.edgeFrom(i);
    const to = this.edgeTo(i);
    const cover = [];
    const level = this.getCovering(from, to, true, cover);
    this.minimumS2LevelUsed = Math.min(this.minimumS2LevelUsed, level);
    cover.forEach(cellId => cellList.push(cellId.id))
    // for (S2CellId cellId : cover) {
    //   cellList.add(cellId.id());
    //   edgeList.add(i);
    // }
  }
  this.cells = new Array(cellList.size());
  this.edges = new int[edgeList.size()];
  for (int i = 0; i < this.cells.length; i++) {
    this.cells[i] = cellList.get(i);
    this.edges[i] = edgeList.get(i);
  }
  sortIndex();
...
public final void predictAdditionalCalls(int n) {
  ...
  if (getNumEdges() > 100 && (this.queryCount + n) > 30) {
    computeIndex();
  }
}
...
private int binarySearch(long cell, int edge) {
  ...
  while (low <= high) {
    int mid = (low + high) >>> 1;
    int cmp = compare(this.cells[mid], this.edges[mid], cell, edge);
    if (cmp < 0) {
      low = mid + 1;
    } else if (cmp > 0) {
      high = mid - 1;
...
private void getEdgesInParentCells(List<S2CellId> cover, Set<Integer> candidateCrossings) {
  ...
  for (S2CellId coverCell : cover) {
    for (int parentLevel = coverCell.level() - 1; parentLevel >= this.minimumS2LevelUsed;
    --parentLevel) {
