...
export class S2Loop implements S2Region/*, Comparable<S2Loop> */{
   ...
   * colinear.
   */
  public static MAX_INTERSECTION_ERROR = 1e-15;
...
public boolean contains(S2Loop b) {
  ...
  if (this.bound.union(b.getRectBound()).isFull()) {
...
public boolean contains(S2Point p) {
  ...
  S2EdgeUtil.EdgeCrosser crosser = new S2EdgeUtil.EdgeCrosser(origin, p,
      this.vertices[this.numVertices - 1]);
  ...
  if (this.numVertices < 2000) {
    ...
    DataEdgeIterator it = getEdgeIterator(this.numVertices);
    int previousIndex = -2;
...
private void initOrigin() {
  ...
  Preconditions.checkState(this.bound.contains(vertex(1)));
  ...
  this.originInside = false; // Initialize before calling Contains().
...
private int findVertex(S2Point p) {
  if (this.vertexToIndex == null) {
    ...
    for (int i = 1; i <= this.numVertices; i++) {
      this.vertexToIndex.put(vertex(i), i);
    }
  }
  Integer index = this.vertexToIndex.get(p);
  if (index == null) {
    return -1;
  } else {
    return index;
