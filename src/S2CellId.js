...
var parseHex = function parseHex(str) {
    return long_1.Long.fromString(str, false, 16);
};
/**
...
var S2CellId = (function () {
    ...
    Object.defineProperty(S2CellId.prototype, "face", {
        get: function () {
            return this.id.shiftRight(S2CellId.POS_BITS).toNumber();
        },
        enumerable: true,
        configurable: true
    });
    ...
    S2CellId.none = function () {
        return new S2CellId(new long_1.Long(0));
    };
    /**
    ...
    S2CellId.prototype.faceSiTiToXYZ = function (face, si, ti) {
        ...
        var uvVector = S2Vector_1.R2Vector.fromSTVector(new S2Vector_1.R2Vector(kScale * si, kScale * ti));
        return uvVector.toPoint(face);
    };
    S2CellId.lowestOnBitForLevel = function (level) {
        return new long_1.Long(1).shiftLeft(2 * (S2CellId.MAX_LEVEL - level));
    };
    /**
    ...
    S2CellId.prototype.toFaceIJOrientation = function (pi, pj, orientation) {
        ...
        if (orientation != null) {
            ...
            if ((long_1.Long.fromString('0x1111111111111110', false, 16).and(this.lowestOnBit()).notEquals(0))) {
     ...
     */
    S2CellId.prototype.parent = function (level) {
    ...
    S2CellId.fromFacePosLevel = function (face, pos, level) {
        // equivalent to pos | 1
        return new S2CellId(new long_1.Long(face)
            .shiftLeft(S2CellId.POS_BITS)
            .add(pos.or(1))).parent(level);
    ...
    S2CellId.fromPoint = function (p) {
        var face = p.toFace();
        var uv = S2Vector_1.R2Vector.fromPointFace(p, face);
        var i = uv.toSt(0);
        var j = uv.toSt(1);
    ...
    S2CellId.prototype.toPointRaw = function () {
        ...
        var delta = this.isLeaf ? 1 : new long_1.Long(i.val).and(this.id.shiftRight(2)).and(1).notEquals(1) ? 2 : 0;
        // ((i.val ? (((int)id) >>> 2))  & 1  ))
        var si = (i.val << 1) + delta - S2CellId.MAX_SIZE;
        var ti = (j.val << 1) + delta - S2CellId.MAX_SIZE;
    ...
    S2CellId.prototype.id = function () {
        return this.id;
    };
    /** Return true if id() represents a valid cell. */
    S2CellId.prototype.isValid = function () {
        return this.face() < S2CellId.NUM_FACES && ((this.lowestOnBit().and(long_1.Long.fromString('0x1555555555555555', false, 16)).notEquals(0)));
        // return this.face() < NUM_FACES && ((lowestOnBit() & (0x1555555555555555L)) != 0);
    };
    /** Which cube face this cell belongs to, in the range 0..5. */
    S2CellId.prototype.face = function () {
        return this.id.shiftRight(S2CellId.POS_BITS).toNumber();
    ...
    S2CellId.prototype.pos = function () {
        return this.id.and(S2CellId.MAX_UNSIGNED.shiftRight(S2CellId.FACE_BITS));
    ...
    S2CellId.prototype.level = function () {
        ...
        if (x != 0) {
            ...
            x = this.id.shiftRight(32).getLowBits();
        }
    ...
    S2CellId.prototype.parent = function () {
        // assert (isValid() && level() > 0);
        var newLsb = this.lowestOnBit().shiftLeft(2);
        return new S2CellId(this.id.and(newLsb.neg()).or(newLsb));
        // return new S2CellId((id & -newLsb) | newLsb);
    };
    /**
     * Return the cell at the previous level or at the given level (which must be
    ...
    S2CellId.prototype.childBegin = function (level) {
    ...
    S2CellId.prototype.childEnd = function (level) {
        // assert (isValid() && level >= this.level() && level <= MAX_LEVEL);
    ...
    S2CellId.begin = function (level) {
        return S2CellId.fromFacePosLevel(0, new long_1.Long(0), 0).childBegin(level);
    };
    S2CellId.end = function (level) {
        return S2CellId.fromFacePosLevel(5, new long_1.Long(0), 0).childEnd(level);
    };
    /**
    ...
    S2CellId.fromToken = function (token) {
        ...
        var value = new long_1.Long(0);
        for (var pos = 0; pos < 16; pos++) {
            var digit = new long_1.Long(0);
            if (pos < token.length) {
                digit = long_1.Long.fromString(token[pos], false, 16);
                if (digit.equals(-1)) {
     ...
     */
    S2CellId.overflowInParse = function (current, digit) {
    ...
    S2CellId.prototype.getEdgeNeighbors = function () {
        ...
        neighbors.push(S2CellId.fromFaceIJSame(face, i.val, j.val - size, j.val - size >= 0).parent(level));
        neighbors.push(S2CellId.fromFaceIJSame(face, i.val + size, j.val, i.val + size < S2CellId.MAX_SIZE).parent(level));
        neighbors.push(S2CellId.fromFaceIJSame(face, i.val, j.val + size, j.val + size < S2CellId.MAX_SIZE).parent(level));
        neighbors.push(S2CellId.fromFaceIJSame(face, i.val - size, j.val - size, i.val - size >= 0).parent(level));
    ...
    S2CellId.prototype.getAllNeighbors = function (nbrLevel) {
        ...
        for (var k = -nbrSize;; k += nbrSize) {
            ...
            if (k < 0) {
                ...
                output.push(S2CellId.fromFaceIJSame(face, i.val + k, j.val - nbrSize, j.val - size >= 0).parent(nbrLevel));
                output.push(S2CellId.fromFaceIJSame(face, i.val + k, j.val + size, j.val + size < S2CellId.MAX_SIZE).parent(nbrLevel));
            }
            // East, West, and Diagonal neighbors.
            output.push(S2CellId.fromFaceIJSame(face, i.val - nbrSize, j.val + k, sameFace && i.val - size >= 0).parent(nbrLevel));
            output.push(S2CellId.fromFaceIJSame(face, i.val + size, j.val + k, sameFace && i.val + size < S2CellId.MAX_SIZE).parent(nbrLevel));
            if (k >= size) {
                break;
            }
        }
    ...
    S2CellId.fromFaceIJ = function (face, i, j) {
        ...
        var faceL = new long_1.Long(face);
        var n = [new long_1.Long(0), faceL.shiftLeft(S2CellId.POS_BITS - 33)];
    ...
    S2CellId.getBits = function (n, i, j, k, bits) {
        var mask = new long_1.Long(1).shiftLeft(S2CellId.LOOKUP_BITS).sub(1);
        bits = bits.add(new long_1.Long(i)
            .shiftRight(k * S2CellId.LOOKUP_BITS)
            .and(mask)
            .shiftLeft(S2CellId.LOOKUP_BITS + 2));
        // bits += (((i >> (k * LOOKUP_BITS)) & mask) << (LOOKUP_BITS + 2));
        bits = bits.add(new long_1.Long(j)
            .shiftRight(k * S2CellId.LOOKUP_BITS)
            .and(mask)
            .shiftLeft(2));
        // bits += (((j >> (k * LOOKUP_BITS)) & mask) << 2);
        bits = S2CellId.LOOKUP_POS[bits];
    ...
    S2CellId.fromFaceIJWrap = function (face, i, j) {
        ...
        var i = Math.max(-1, Math.min(S2CellId.MAX_SIZE, i));
        var j = Math.max(-1, Math.min(S2CellId.MAX_SIZE, j));
        ...
        var p = new S2Vector_1.R2Vector(s, t).toPoint(face);
        // let face = p.toFace();
    ...
    S2CellId.maxValueDivs = [new long_1.Long(0), new long_1.Long(0),
    ...
    S2CellId.MAX_UNSIGNED = long_1.Long.fromString('0xffffffffffffffff', true, 16);
    ...
    S2CellId.LOOKUP_POS = new [1 << (2 * S2CellId.LOOKUP_BITS + 2)];
    S2CellId.LOOKUP_IJ = new [1 << (2 * S2CellId.LOOKUP_BITS + 2)];
    /**
     * This is the offset required to wrap around from the beginning of the
     * Hilbert curve to the end or vice versa; see next_wrap() and prev_wrap().
     */
    S2CellId.WRAP_OFFSET = new long_1.Long(S2CellId.NUM_FACES).shiftLeft(S2CellId.POS_BITS);
    return S2CellId;
}());
...
function initLookupCell(level, i, j, origOrientation, pos, orientation) {
    if (level == S2CellId.LOOKUP_BITS) {
        ...
        S2CellId.LOOKUP_IJ[(pos << 2) + origOrientation] = new long_1.Long((ij << 2)).add(orientation);
    }
    else {
        level++;
        i <<= 1;
...
initLookupCell(0, 0, 0, 0, new long_1.Long(0), 0);
initLookupCell(0, 0, 0, S2_1.S2.SWAP_MASK, new long_1.Long(0), S2_1.S2.SWAP_MASK);
initLookupCell(0, 0, 0, S2_1.S2.INVERT_MASK, new long_1.Long(0), S2_1.S2.INVERT_MASK);
initLookupCell(0, 0, 0, S2_1.S2.SWAP_MASK | S2_1.S2.INVERT_MASK, new long_1.Long(0), S2_1.S2.SWAP_MASK | S2_1.S2.INVERT_MASK);
